...
var Candlestick = function (element, settings) {
    ...
    if (this.settings.mode == 'contents')
...
Candlestick.prototype.wrapElement = function () {
    ...
    switch (this.settings.mode) {
        case 'contents':
            var html = '<div class="candlestick-wrapper candlestick-contents"><div data-candlestick-id="' + $id + '" class="candlestick-bg' + $classes + '"><div class="candlestick-toggle"></div><div class="candlestick-off" data-content="' + this.settings.contents.left + '"></div><div class="candlestick-nc" data-content="' + this.settings.contents.middle + '"></div><div class="candlestick-on" data-content="' + this.settings.contents.right + '"></div><input type="hidden" class="' + this.element.attr('class') + '" value="' + this.default.value + '" name="' + this.default.name + '" id="' + $id + '"' + datas + $inputDisabled + $inputReadonly + '></div></div>';
        break;
        default:
        ...
        if (this.settings.mode == "contents") {
            ...
            .on('mousemove', function(e) {
                if ($(this).hasClass('move')) {
                    var leftElement = parseInt($(this).find('.candlestick-toggle').css('left'));

                    if (e.offsetX < this.offsetX) { // LEFT
                        var move = e.offsetX - this.offsetX;
                    }
                    else { // RIGHT
                        var move = e.offsetX + this.offsetX;
                    }

                    $(this).find('.candlestick-toggle').css('left', leftElement + move);
                    console.log((leftElement + move) + ' - ' + leftElement + ' - ' + move);
                }
            })
...
Candlestick.prototype.actionSwipe = function () {
    ...
    if (this.isSwipeEnable()) {
        if (typeof $.fn.hammer != 'undefined') {
            this.parent.find('.candlestick-toggle').hammer({
                threshold: 0,
                velocity: 0.1
            })
